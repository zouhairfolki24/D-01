...
void affiche_joueurs_plusbuts(struct Joueur joueurs[], int nombre_joueurs, int x_buts){
    for (int i = 0; i < nombre_joueurs; i++) {
        if (joueurs[i].buts >= x_buts) {
            printf("\nID: %d, Nom: %s, Prenom: %s, Numero Maillot: %d, Poste: %s, Age: %d, Buts: %d.\n", joueurs[i].id, joueurs[i].nom, joueurs[i].prenom, joueurs[i].numeroMaillot, joueurs[i].poste, joueurs[i].age, joueurs[i].buts);
        }
    }
}   
...
void affiche_jeune_plus(struct Joueur joueurs[], int nombre_joueurs) {
    ...
    int max_age = joueurs[0].age;
    int min_age = joueurs[0].age;
    for (int i = 1; i < nombre_joueurs; i++) {
        if (joueurs[i].id != 0) {
    ...
    for (int i = 0; i < nombre_joueurs; i++) {
        if (joueurs[i].id != 0 && joueurs[i].age == max_age){
            printf("\nLe joueur le plus age: %d\n", max_age);
        } else if (joueurs[i].id != 0 && joueurs[i].age == min_age){
            printf("\nLe joueur le plus jeune: %d\n", min_age);
        }
    }
        ...
        case '6':
            ...
            while (check_6)
            {
                ...
                case '3':
                    ...
                    nombre_joueurs = calcule_nbr_joueurs(joueurs, 100);
                    affiche_joueurs_plusbuts(joueurs, nombre_joueurs, x_buts);

                    break;
                case '4':
                    // Afficher le meileur buteur

                    nombre_joueurs = calcule_nbr_joueurs(joueurs, 100);
                    affiche_meilleur_butteur(joueurs, nombre_joueurs);

                    break;
                case '5':
                    // Afficher le joueur plus jeune et plus age.

                    nombre_joueurs = calcule_nbr_joueurs(joueurs, 100);
                    affiche_jeune_plus(joueurs, nombre_joueurs)
