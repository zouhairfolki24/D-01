...
void ajouter_joueur(struct Joueur joueurs[], int index) {
    ...
    char nomComplet[30], poste[30];
    int numeroMaillot, age, buts;

    printf("\nNom Complet: ");
    fgets(nomComplet, sizeof(nomComplet), stdin);
    nomComplet[strcspn(nomComplet, "\n")] = 0;
    ...
    joueurs[index].id = id;
    strcpy(joueurs[index].nomComplet, nomComplet);
...
void trier_noms(int noms[], int size_joueurs) {
    
}

void affiche_joueurs_nom(struct Joueur joueurs[], int size_joueurs) {

    int noms[size_joueurs][30];
    int j = 0;
    for (int i = 0; i < size_joueurs; i++) {
        if (strcmp(joueurs[i].nomComplet, "\0") != 0) {
            noms[j] = joueurs[i].nomComplet;
            strcpy(noms[j], joueurs[i].nomComplet)
            j++;
        }
    }
