...
void ajoute_contact(struct Contact contacts[], struct Contact contact, int len_contacts) {
    for (int i = 0; i < len_contacts; i++) {
        if (strcmp(contacts[i].nom, "\0") != 0 && strcmp(contacts[i].numero_telephone, "\0") != 0 && strcmp(contacts[i].nom, "\0") != 0)){
            contacts[i] = contact;
        }
    }
}
    ...
    while (true) {
        ...
        switch (choix) {
            ...
            case 1:
                ...
                ajoute_contact(contacts, contact, 100)
                break;
