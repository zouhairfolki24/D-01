...
int main()
{
    ...
    int index_titre = -1;
    char titre_supprimer[50];
    int index_item_supprimer = -1;
    int total_quantites = 0;
    ...
    while (true) {
        ...
        switch (choix_user) {
            ...
            case '5':
                ...
                fgets(titre_supprimer, sizeof(titre_supprimer), stdin);
                titre_supprimer[strcspn(titre_supprimer, "\n")] = 0;

                for (int i = 0; i < 100; i++) {
                    if (strcmp(titre_supprimer, titres[i]) == 0) {
                        index_item_supprimer = i;
                    }
                }

                if (index_item_supprimer != -1) {
                    strcpy(titres[index_item_supprimer], "\0");
                    strcpy(auteurs[index_item_supprimer], "\0");
                    prix[index_item_supprimer] = 0.0;
                    quantites[index_item_supprimer] = 0;

                    index_item_supprimer = -1;
                } else {
                    printf("\nLivre non trouve.\n");
                }
            ...
            case '6':
                for (int i = 0; i < 100; i++) {
                    if (quantites[i] == 0) {
                        total_quantites += quantites[i];
                    }
                }

                printf("Le nombre total de livres en stock est: ")
